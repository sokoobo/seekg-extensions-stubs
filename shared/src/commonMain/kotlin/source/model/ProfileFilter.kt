...
sealed class ProfileFilter<T>(val name: String, var state: T) {
     ...
     */
    abstract class Select<V>(name: String, val values: Array<V>, state: Int = 0) : ProfileFilter<Int>(name, state)
     ...
     */
    abstract class CheckBox(name: String, state: Boolean = false) : ProfileFilter<Boolean>(name, state)
     ...
     */
    abstract class TriState(name: String, state: Int = STATE_IGNORE) : ProfileFilter<Int>(name, state) {
